...
export const generatePinContentFromKeywordWithOpenRouter = async (apiKey: string, model: string, keyword: string, boardOptions: string = "", categoryOptions: string = ""): Promise<Omit<GeneratedContentRow, 'keyword'>> => {
    const prompt = `Generate Pinterest pin content for the keyword: "${keyword}".
    Return a valid JSON object with: title, description, board${boardOptions ? ` (choose from: ${boardOptions})` : ''}, image_prompt, alt_text, interests (array), category${categoryOptions ? ` (choose from: ${categoryOptions})` : ''}.`;
...
export const rewriteKeyword = async (apiKey: string, model: string, keyword: string): Promise<string> => {
    ...
    const response = await ai.models.generateContent({
        model: model || 'gemini-2.5-flash',
        contents: `Rewrite the keyword "${keyword}" to be more specific and search-friendly for Pinterest. Return only the new keyword.`,
    });
...
export const rewriteKeywordWithOpenRouter = async (apiKey: string, model: string, keyword: string): Promise<string> => {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
            model: model,
            messages: [{ role: "user", content: `Rewrite the keyword "${keyword}" to be more specific and search-friendly. Return only the new keyword.` }]
        })
    });
    const data = await response.json();
...
export const rewriteDescriptionWithOpenRouter = async (apiKey: string, model: string, title: string, description: string, categoryOptions: string = ""): Promise<{title: string, description: string, category: string, keywords: string}> => {
...
export const generatePinIdeas = async (apiKey: string, model: string, accountName: string): Promise<PinIdea[]> => {
    ...
    const response = await ai.models.generateContent({
        model: model || 'gemini-2.5-flash',
        contents: `Generate 5 fresh Pinterest pin ideas for a profile named "${accountName}". Return JSON array of objects with title, description, and hashtags fields.`,
        config: {
            responseMimeType: 'application/json',
            responseSchema: {
                type: Type.ARRAY,
...
export const getAiSuggestions = async (apiKey: string, model: string, account: PinterestAccount): Promise<AISuggestions> => {
    ...
    const response = await ai.models.generateContent({
        model: model || 'gemini-2.5-flash',
        contents: `Analyze this Pinterest account: Name="${account.name}", Notes="${account.notes}". Suggest the best time to post, the next pin type to create, and a seasonal theme. Return JSON.`,
        config: {
            responseMimeType: 'application/json',
            responseSchema: {
                type: Type.OBJECT,
...
export const generateFacebookPost = async (apiKey: string, model: string, topic: string): Promise<FacebookPost> => {
    ...
    const response = await ai.models.generateContent({
        model: model || 'gemini-2.5-flash',
        contents: `Create a Facebook post about "${topic}". Return JSON with: postText (engaging text), imagePrompt (for AI image gen), hashtags (array of strings), imageText (short text to overlay on image).`,
...
export const generateFacebookPostWithOpenRouter = async (apiKey: string, model: string, topic: string): Promise<FacebookPost> => {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
            model: model,
            messages: [{ role: "user", content: `Create a Facebook post about "${topic}". Return JSON with: postText, imagePrompt, hashtags (array), imageText.` }],
            response_format: { type: "json_object" }
        })
    });
    const data = await response.json();
...
export const generateFacebookPageStrategy = async (apiKey: string, model: string, niche: string, country: string): Promise<FacebookPageStrategy> => {
    ...
    const response = await ai.models.generateContent({
        model: model || 'gemini-2.5-flash',
        contents: `Create a Facebook page strategy for Niche: "${niche}", Country: "${country}". Return detailed JSON matching the FacebookPageStrategy interface including page_name_ideas, page_bio_90chars, categories, logo_brief, cover_brief, public_group, page_likes_ad, first_20_post_themes.`,
        config: { responseMimeType: 'application/json' }
    });
    return JSON.parse(response.text || '{}');
};
...
export const generateViralQuotes = async (apiKey: string, model: string, category: string, length: 'short' | 'long'): Promise<string[]> => {
    ...
    const response = await ai.models.generateContent({
        model: model || 'gemini-2.5-flash',
        contents: `Generate 5 viral, aesthetic quotes about "${category}". Length: ${length}. Return JSON array of strings.`,
        config: { responseMimeType: 'application/json' }
    });
    return JSON.parse(response.text || '[]');
};
...
export const generateViralQuotesWithOpenRouter = async (apiKey: string, model: string, category: string, length: 'short' | 'long'): Promise<string[]> => {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
            model: model,
            messages: [{ role: "user", content: `Generate 5 viral quotes about "${category}". Length: ${length}. Return JSON array of strings under key "quotes".` }],
            response_format: { type: "json_object" }
        })
    });
    const data = await response.json();
...
export const enhanceViralQuote = async (apiKey: string, model: string, quote: string, length: 'short' | 'long'): Promise<string[]> => {
    ...
    const response = await ai.models.generateContent({
        model: model || 'gemini-2.5-flash',
        contents: `Rewrite this quote to be more viral and aesthetic: "${quote}". Length: ${length}. Provide 3 variations. Return JSON array of strings.`,
        config: { responseMimeType: 'application/json' }
    });
    return JSON.parse(response.text || '[]');
};
...
export const enhanceViralQuoteWithOpenRouter = async (apiKey: string, model: string, quote: string, length: 'short' | 'long'): Promise<string[]> => {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
            model: model,
            messages: [{ role: "user", content: `Rewrite quote to be viral: "${quote}". Length: ${length}. 3 variations. Return JSON array of strings under key "quotes".` }],
            response_format: { type: "json_object" }
        })
    });
    const data = await response.json();
...
export const generateSoraVideoPrompt = async (apiKey: string, model: string, quote: string): Promise<string> => {
    ...
    const response = await ai.models.generateContent({
        model: model || 'gemini-2.5-flash',
        contents: `Create a detailed, cinematic video generation prompt (for OpenAI Sora) that visually represents the mood of this quote: "${quote}". Describe lighting, camera movement, subject, and atmosphere.`,
    });
...
export const generateSoraVideoPromptWithOpenRouter = async (apiKey: string, model: string, quote: string): Promise<string> => {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
            model: model,
            messages: [{ role: "user", content: `Create a cinematic video prompt for Sora based on quote: "${quote}".` }]
        })
    });
    const data = await response.json();
    return data.choices[0].message.content.trim();
