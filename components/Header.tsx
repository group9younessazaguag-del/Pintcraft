...
const NavLink: React.FC<{ href: string; children: React.ReactNode; onClick?: () => void; isActive: boolean }> = ({ href, children, onClick, isActive }) => (
    <a 
      href={href} 
      onClick={onClick}
      className={`flex items-center h-16 px-3 transition-all duration-200 text-sm font-medium border-b-2 whitespace-nowrap flex-shrink-0 ${isActive ? 'text-pink-500 font-semibold border-pink-500' : 'text-slate-600 dark:text-slate-400 hover:text-pink-500 dark:hover:text-pink-400 border-transparent'}`}
    >
        {children}
    </a>
);
...
const MobileNavLink: React.FC<{ href: string; children: React.ReactNode; onClick?: () => void; isActive: boolean }> = ({ href, children, onClick, isActive }) => (
    <a 
      href={href}
      onClick={onClick}
      className={`block px-4 py-3 text-lg rounded-lg transition-colors duration-200 ${isActive ? 'bg-pink-50 dark:bg-pink-900/20 text-pink-600 dark:text-pink-400 font-semibold' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-pink-500 dark:hover:text-pink-400'}`}
    >
        {children}
    </a>
);
...
const Header: React.FC = () => {
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 md:px-8">
          <div className="flex items-center h-16">
            ...
            <div className="flex-shrink-0 mr-6">
                ...
                <h1 className="text-xl font-semibold tracking-tight text-slate-800 dark:text-slate-100 transition-colors group-hover:text-slate-900 dark:group-hover:text-white whitespace-nowrap">
                    Pin4You
                </h1>
                </a>
            </div>
            ...
            <div className="hidden md:flex flex-1 min-w-0 overflow-hidden justify-end items-center gap-4">
                <nav 
                ...
                >
                    ...
                    {navItems.map(item => (
                        ...
                        key={item.href}
                        href={item.href}
                        isActive={activePage === (item.href.substring(3) || 'pin-generator')}
                        >
                            {item.label}
                        </NavLink>
                    ))}
            ...
            {/* Mobile Menu Button */}
            <div className="md:hidden ml-auto flex-shrink-0 flex items-center gap-2">
              <ThemeToggle />
              <button 
                onClick={toggleMobileMenu} 
                className="p-2 rounded-md text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white focus:outline-none focus:ring-2 focus:ring-pink-500 transition-colors"
      ...
      {/* Mobile Menu Panel */}
      <div 
        className={`fixed top-0 right-0 h-full w-4/5 max-w-sm bg-white dark:bg-slate-900 z-50 shadow-2xl transform transition-transform duration-300 ease-in-out md:hidden ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
        role="dialog"
        aria-modal="true"
      >
