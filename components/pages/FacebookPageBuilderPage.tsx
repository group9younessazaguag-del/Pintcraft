...
export const FacebookPageBuilderPage: React.FC<FacebookPageBuilderPageProps> = ({
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            setApiError(err.message || "An unknown error occurred while generating the strategy.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="container mx-auto max-w-7xl">
            <div className="text-center mb-8">
                <h1 className="text-4xl font-bold tracking-tight text-slate-800 dark:text-slate-100">AI Facebook Page Builder</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <ControlCard icon={<FacebookIcon />} title="1. Define Your Page">
                        <div>
                            <label htmlFor="niche" className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1.5">Niche</label>
                            <input
                                type="text"
                                id="niche"
                                value={niche}
                         ...
                         <div>
                            <label htmlFor="country" className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1.5">Primary Country</label>
                            <input
                                type="text"
                                id="country"
                                value={country}
                            ...
                            getLinkText="Get an OpenRouter API Key"
                             statusMessage={
                                openRouterKeyIsConfigured ? (
                                    ...
                                    <p className="text-amber-800 dark:text-amber-100 bg-amber-50 dark:bg-amber-900/30 p-2 rounded-lg border border-amber-200 dark:border-amber-800 font-medium"><strong>Required</strong> for strategy generation.</p>
                                )
                            }
                        />
                         <div>
                ...
                {/* Strategy Display Column */}
                 <div className="lg:col-span-2 space-y-6">
                    ...
                    {isLoading && (
                        <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-200/80 dark:border-slate-800">
                            <LoadingSpinner className="w-10 h-10" />
                            <p className="mt-4 text-lg font-medium text-slate-800 dark:text-slate-100">Building your page strategy...</p>
                            <p className="text-sm">This may take a moment.</p>
                        </div>
                    )}

                    {!isLoading && !strategy && (
                        <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-200/80 dark:border-slate-800">
                    ...
                    {strategy && (
                        <>
                             <StrategySection icon={<LightbulbIcon className="w-6 h-6"/>} title="Page Name Ideas & Categories">
                                ...
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-3 font-semibold">Categories: {strategy.categories.join(', ')}</p>
                            </StrategySection>

                            <StrategySection icon={<ProfileIcon className="w-6 h-6"/>} title="Page Bio (90 Chars)">
                                <p className="text-slate-700 dark:text-slate-300 italic">"{strategy.page_bio_90chars}"</p>
                            </StrategySection>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <StrategySection icon={<PaletteIcon className="w-6 h-6"/>} title="Logo Brief">
                                    <div className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
                                        <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Style:</strong> {strategy.logo_brief.style}</p>
                                        <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Motifs:</strong> {strategy.logo_brief.motifs.join(', ')}</p>
                                        <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Notes:</strong> {strategy.logo_brief.notes}</p>
                                    </div>
                                </StrategySection>
                                <StrategySection icon={<PaletteIcon className="w-6 h-6"/>} title="Cover Photo Brief">
                                     <div className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
                                        <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Concept:</strong> {strategy.cover_brief.concept}</p>
                                        <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Layout Notes:</strong> {strategy.cover_brief.layout_notes}</p>
                                    </div>
                                </StrategySection>
                            </div>
                            ...
                            <StrategySection icon={<GroupIcon className="w-6 h-6"/>} title="Public Group Strategy">
                                <div className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
                                    <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Name:</strong> {strategy.public_group.name}</p>
                                    <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Description:</strong> {strategy.public_group.description}</p>
                                    <p><strong className="font-semibold text-slate-800 dark:text-slate-200">First Pinned Post:</strong> {strategy.public_group.first_pinned_post}</p>
                                </div>
                            </StrategySection>

                            <StrategySection icon={<ActionsIcon className="w-6 h-6"/>} title="Page Likes Ad Campaign">
                                 <div className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
                                    <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Headline:</strong> {strategy.page_likes_ad.headline}</p>
                                    <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Primary Text:</strong> {strategy.page_likes_ad.primary_text}</p>
                                    <p className="font-mono text-xs bg-slate-100 dark:bg-slate-800 p-2 rounded-md"><strong className="font-semibold text-slate-800 dark:text-slate-200">Image Prompt:</strong> {strategy.page_likes_ad.image_prompt}</p>
                                    <p><strong className="font-semibold text-slate-800 dark:text-slate-200">Placements:</strong> {strategy.page_likes_ad.placements.join(', ')}</p>
                                </div>
                            </StrategySection>

                             <StrategySection icon={<ContentIcon className="w-6 h-6"/>} title="First 20 Post Themes">
                                <ol className="list-decimal list-inside space-y-1.5 text-slate-700 dark:text-slate-300 text-sm grid grid-cols-1 md:grid-cols-2 gap-x-4">
